// https://css-tricks.com/typewriter-animation-that-handles-anything-you-throw-at-it/#top-of-site
function typing_animation() {
    ...
    if (text_array_slice.length == 1) {
        ...
        let reveal_animation = document.querySelector(".text_hide").animate([
            { left: '0%' },
            { left: `${(percentage / text_len) * (word_len[0])}%` }
        ], timings);
        ...
        document.querySelector(".text_hide").animate([
            { left: '0%' },
            { left: `${(percentage / text_len) * (word_len[0] + 1)}%` }
        ], timings);
